...
type Mode = "processes" | "cve" | "events";
...
const parseArgs = (): ParsedArgs => {
  ...
  if (!mode || !["processes", "cve", "events"].includes(mode)) {
    throw new Error(
      "Missing or invalid --mode. Expected one of 'processes', 'cve', 'events'."
    );
  }
  ...
  let format = options.format as "csv" | "json" | undefined;
...
const upsertProcess = async (record: ProcessRecord) => {
...
const upsertCveRecord = async (record: CVERecordInput) => {
  if (!client) throw new Error("Database client not initialized");
  await client.query(
    `
...
const insertNetworkEvent = async (record: NetworkEventRecord) => {
  if (!client) throw new Error("Database client not initialized");
  await client.query(
    `
...
const validateProcessRecord = (record: ProcessRecord): void => {
  ...
  const tasks = (record.definition as any)?.tasks;
...
const validateCveRecord = (record: CVERecordInput): void => {
  if (!record.orgId || !record.projectId) {
    throw new Error("CVE record missing orgId/projectId.");
  }
  if (!record.cveId) {
...
const validateEventRecord = (record: NetworkEventRecord): void => {
  if (!record.orgId || !record.projectId) {
    throw new Error("Event record missing orgId/projectId.");
  }
  if (!record.source) {
...
async function run() {
  ...
  const rows =
    options.format === "csv" ? parseCsvContent(content) : parseJsonContent(content);
    ...
    try {
      if (options.mode === "processes") {
        ...
        if (options.dryRun) {
          ...
          const taskCount = Array.isArray((record.definition as any)?.tasks) 
            ? (record.definition as any).tasks.length 
            : 0;
