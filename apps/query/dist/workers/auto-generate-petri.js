...
import { fetchActivityCounts, generatePetriNetStructure, } from "../lib/petri.js";
...
const selectProcessesNeedingNets = async () => {
    if (!client)
    ...
    return res.rows.map((row) => ({
        ...row,
        latest_net: row.latest_net,
    }));
};
const insertPetriNet = async (process, structure, version) => {
    if (!client)
        throw new Error("Database client not initialized");
    const id = randomUUID();
    await client.query(`
...
async function run() {
    ...
    if (!DATABASE_URL || !client) {
        console.error("Database connection required for non-dry-run mode.");
        process.exit(1);
    }
    ...
    for (const process of processes) {
        try {
            ...
            if (!structure.nodes.length) {
                console.warn(`Skipping process ${process.id} (${process.name}) because definition has no tasks`);
                continue;
            }
            const nextVersion = (process.latest_net?.version ?? 0) + 1;
            const netId = await insertPetriNet(process, structure, nextVersion);
            console.log(`• Created Petri net ${netId} for process ${process.name} (v${nextVersion})`);
        }
        catch (error) {
            console.error(`Failed to generate net for process ${process.id}:`, error);
        }
    }
    await client.end();
}
